...
class ResidueNa( ):
    ...
    def get_chi2( self, params ):
        ...
        eff_avg = self.data.values[key1].Yield[0]/( ( 1 - eff.effTot( 1.27457 ) )*1.807 )
        ...
        residues.append( pow( ( Yield - self.data.values[key2].Yield[0] )/self.data.values[key2].YieldErr[0], 2 ) )
        return residues
    ...
    def get_disc( self, params ):
        ...
        eff_tot_avg = eff_avg/( np.exp( params["k1"] + params["k2"]*np.log( 0.511 ) + params["k3"]*pow( np.log( 0.511 ), 2 ) ) )
        Yield = eff.effPeak( float( key2 )/1000 )*( 1 - 1.807*eff_tot_avg )
        disc.append( ufloat( ( Yield - self.data.values[key2].Yield[0] )/self.data.values[key2].Yield[0], 0 ) )
        return disc
    ...
    def get_data( self ):
        key = "1274.57"
        data = [ ufloat( self.data.values[key].Yield[0], self.data.values[key].YieldErr[0] ) ]
        return data
